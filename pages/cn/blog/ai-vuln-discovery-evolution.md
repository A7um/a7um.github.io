...
abstract: >
  2025 年，我们的系统已经自动发现了 60+ 个真实世界漏洞，覆盖密码学库、AI 框架和云服务。回顾这段历程，我们发现：**成功的关键并非某个单一技术突破，而是正确把握了 AI 演进的范式，并在每次范式转换中及时调整方法**。与此同时，我们也观察到，大量曾经发表于顶会的工作，因为未能跟上范式转移而逐渐失去现实影响力。这一现象促使我们写下这篇文章：系统梳理 2022–2025 年间自动化漏洞挖掘的三次范式跃迁——从「LLM 做代码分类」到「LLM 辅助传统工具」再到「Agent 主导的自动化审计」——帮助读者理解范式转换的规律，做出能够跨越范式的研究与工程决策。
---

2025 年，我们的系统已经自动发现了超过 60 个真实世界的安全漏洞。这些漏洞涵盖密码学库、AI 框架和云服务三大领域。在密码学领域，我们在 Rust-Crypto、BouncyCastle、gnark 等主流库中发现了十余个漏洞，包括私钥恢复（CVE-2026-22698，CVSS 8.7）、密钥重用导致消息解密（CVE-2025-14813，CVSS 8+）、签名可塑性（CVE-2025-57801，CVSS 9.1）等高危问题。在 AI 领域，我们在 MaxB、Flowise、Ragflow 等框架中发现了十余个远程代码执行漏洞，包括任意文件写入 RCE（CVE-2025-61913，CVSS 9.9）、信息泄漏结合 SSRF 导致的 RCE（CVE-2025-64703、CVE-2025-64511）等。在云安全领域，我们也在 Tyk 等主流云产品上发现了十余个严重问题，正在走负责任披露流程。

回顾这段历程，我反复体会到一个现象：我们之所以能走到今天，并非因为某个单一技术的突破，而是因为**正确把握了 AI 演进的范式，并将最新技术与自身创新方法相结合**。我们在论文阅读时也观察到，有相当一部分工作未能及时适应范式转换——即便曾在顶会发表，也逐渐被 AI 的快速发展所超越。
...
- 随着 CFG、RFG、MTE、PAC 等缓解措施的普及，攻防格局也在改变。MTE 压缩了堆溢出的利用窗口，CFI 阻断了控制流劫持的经典路径，PAC 让 ROP 链构造成本急剧上升。许多崩溃即使被发现也难以转化为可利用漏洞，从 crash 到 exploit 的鸿沟正在持续拉大。
...
<center>图：基于污点分析的自动化漏洞挖掘的典型工作流与局限。</center>
...
<center>图：符号执行与形式化验证在自动化漏洞挖掘系统中的典型位置。</center>
...
<center>图：理想的自动化漏洞挖掘系统在通用性、自动化、低误报和低漏报四个维度上的目标。</center>
...
<center>图：2022 年主流代码模型的典型能力边界。</center>
...
<center>图：2023 年在世界知识、函数级理解和代码生成三方面的能力提升。</center>
...
<center>图：2023 年的经验教训——将 LLM 放在“合适的位置”。</center>
...
<center>图：2024 年在上下文、推理和 Agent 框架上的关键变化。</center>
...
<center>图：2024 年的核心启示——识别范式转换，比在旧范式里打磨细节更重要。</center>
...
<center>图：2025 年 Agent 能力成熟——长程规划、工具编排和自我纠错成为标配。</center>
...
<center>图：Agent 驱动的自动化审计体系：LLM 负责提出假设与决策，fuzzer、静态分析器、符号执行器等作为可调用工具。</center>
...
- **静态分析方向**  
  *LLM-Driven SAST-Genius: A Hybrid Static Analysis Framework*（IEEE S&P 2025）使用 LLM 将 Semgrep 误报从 225 个压缩到 20 个（减少 91%），并生成 exploit 验证漏洞。QLPro: Automated Code Vulnerability Discovery 使用 LLM 生成 source/sink 识别规则和 CodeQL 规则，在 10 个开源项目中检测到 41 个漏洞（CodeQL 只检测到 24 个），发现 6 个未知漏洞，其中 2 个确认为 0‑day。
...
<center>图：在自动化漏洞挖掘中，真正稀缺的是知识，而非算力或模型规模。</center>
...
同样是 Claude 4.5，一个设计拙劣的 Agent 可能项目规模稍大一点就无法完成；而 Cursor 却展示了，一个精心编排的 Agent 系统，可以持续运行超过 10 周，并从零开始写出一个 100 万行代码的浏览器。差距不在模型，在编排。
...
研究的回报模式正在变化。一年前的 state-of-the-art，今天可能已经不再是最优解，因为底层范式已经转移。在这样的环境下，理解趋势、预见范式变得越来越重要；关注实际效果，而非仅仅追求论文发表，也变得越来越重要。我们能靠系统自动发现 60+ 个真实漏洞，并不是因为论文数量，而是因为长期追问一个问题：**什么真正有效？**
