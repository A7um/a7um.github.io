...
Playing with the challenge binary, we will see several options such as Open, Read, Write, Close. It looks like this challenge is functionally related to file operations.
...
By now, everything seems to be OK, no logic error, no UAF, no vulnerability ?
...
With the ability to overwrite FILE structure, we have several  choices to hijack the control flow, such as overwrite IO\ file\ jump pointer(despite the libc2.24 add the vtable check, we still can achieve this if we can control some field of the FILE structure, see this) or overwrite the input buffer inside the FILE structure. The solution seems to get clear.
...
```
The code above is the FILE structure. we can see there is 8 pointer related to buffering. for the info leak, we first overwrite the last byte of \_IO\_write\_base to 0x00 using partial overwrite, then close the overwrited FILE structure using fclose(), since fclose() will flush the buffer, all the data from \_IO\_write\_base to \_IO\_write\_ptr will be ouputed into FILE.

But there still something challenging: All the file we opened in the chanllenge is opened in readonly mode, so the techniques we state above will not work because write to a read only file descriptor  will fail. the solution is overwriting  file descriptor  field of FILE structure to 1(stdout) or 2(stderr) first. this will also cause a sidework  that all buffer related field will also be overwrited and we can no longer perform a partial overwrite. this stunk us a little while, but quickly we figure out that fread and fseek will try to fix an  invalid buffer. so we can use read in the menu to fix the buffer pointers inside the FILE structure. and finally we can perform partial  overwrite to the \_IO\_write\_base and close the file, a heap address will be leaked.
...
Note that, since we closed both stdout and stderr during the leak stage. so after we get /bin/sh running, we need to spawn a reverse shell to get an interactive shell.

### Attack remotely

once we get the local shell, we continue trying to attack remotely. after several  unsuccessfully try. we suddenly realize that there is something we forgot. the readme file inside the challenge package.
...
What is so called ",pty,ctty,echo=0"? to figure it out, we  use a local socat server and try to attack "remotely", then attach the debugger to the challenge process and try to find out why it failed remotely. we found surprisingly  that all the '\xff\x7f' are disappear ! it seems like the console parses the '\x7f' as delete and "eat" the '\xff'. this makes  us crazy for sure because we takes several  hours to figure out how to escape the '\x7f'. Finally we know the SYNC control character '\x16' can escape that and prevent the character before '\x7f' to be eaten after the contest end. Yes! after the contest end QaQ.
