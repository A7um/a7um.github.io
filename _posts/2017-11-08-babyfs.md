...
once we get the local shell, we continue trying to attack remotely. after some unsuccessfully try. we suddenly realize that there is something we forgot. the readme file inside the challenge package.
...
What is so called ",pty,ctty,echo=0"? to figure it out, we  use a local socat server and try to attack "remotely", then attach the debugger to the challenge process and try to find out why it failed remotely. we found surprisingly  that all the '\xff\x7f' are disappear ! it seems like the console parses the '\x7f' as delete and "eat" the '\xff'. this makes  us crazy for sure because we takes a lot time to figure out how to escape the '\x7f'. Finally we know the SYNC control character '\x16' can escape that and prevent the character before '\x7f' to be eaten after the contest end. Yes! after the contest end QaQ.
